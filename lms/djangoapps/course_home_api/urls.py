"""
Contains all the URLs for the Course Home
"""


from django.conf import settings
from django.urls import re_path

from lms.djangoapps.course_home_api.dates.v1.views import DatesTabView
from lms.djangoapps.course_home_api.general.v1.views import CourseHomeView

urlpatterns = []

# General URL for every page content
urlpatterns += [
    re_path(
        r'v1/general/{}'.format(settings.COURSE_KEY_PATTERN),
        CourseHomeView.as_view(),
        name='course-home-general'
    ),
]

# Dates Tab URLs
urlpatterns += [
    re_path(
        r'v1/dates/{}'.format(settings.COURSE_KEY_PATTERN),
        DatesTabView.as_view(),
        name='course-home-dates-tab'
    ),
]
